	...
	<body>
		<div class="sidebar">
			...
			<ul class="sideList">
				...
				<a href="reports.php">
					<li class="list-item active"><i class="fa fa-file"></i><span style="font-size:16px;">Reports</span></li>	
				</a>
			</ul>
		</div>
		<div class="navbar">
			...
			<div class="bg-white p-4" style="min-height:82vh;">
				...
				<table id="studentList2" class="table table-striped table-bordered bg-white" style="width:100%;">
					...
					<?php
						...
						if(!$con)
							die("Error");
						$q1="select s1.id, s1.name from student s1 where not exists(select null from fees f2 where f2.stdID = s1.id)";
						$q1_run=mysqli_query($con,$q1);
						$rows=mysqli_num_rows($q1_run);
						if($rows > 0){
						while($row2=mysqli_fetch_assoc($q1_run)) {
						?>
							<tr>
